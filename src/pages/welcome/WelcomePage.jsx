...
const WelcomePage = () => {
  ...
  const handleExperienceSubmit = async () => {
    ...
    setIsSubmitting(true);
    try {
      // Try to create guest on backend
      try {
        const response = await fetch("http://localhost:5000/api/v1/guest/register", {
          method: "POST",
          headers: { "Content-Type": "application/json" },
        ...
        if (response.ok) {
          const data = await response.json();
          console.log("Guest created on backend:", data);
          localStorage.setItem("guestId", data.guestId);
          localStorage.setItem("guestExperience", data.experience);
          navigate("/Explora/home");
          return;
        ...
        console.warn("Backend unavailable, using local guest creation", backendError);
      }

      // Fallback: Create guest locally
      const guestId = `Guest_${Date.now()}_${Math.random().toString(36).substr(2, 9)}`;
      console.log("Guest created locally:", guestId);
      
      localStorage.setItem("guestId", guestId);
      localStorage.setItem("guestExperience", experience);
